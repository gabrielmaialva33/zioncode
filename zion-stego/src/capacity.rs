...
/// ```
#[must_use]
pub fn photo_k_per_photo(width: u32, height: u32, density: EmbeddingDensity) -> u16 {
    ...
    let embeddable_bits =
        (permutable as u64) * u64::from(density.scaled_parts_per_100k()) / 100_000;
...
/// `dims` is empty.
#[must_use]
pub fn compute_k_global(dims: &[(u32, u32)], density: EmbeddingDensity) -> Option<u16> {
    if dims.is_empty() {
        return None;
    }
...
mod tests {
    ...
    fn default_density() -> EmbeddingDensity {
        EmbeddingDensity::new(EMBEDDING_DENSITY_DEFAULT).unwrap()
    }

    #[test]
    fn k_per_photo_1080p() {
        // Per spec Appendix B: 1920×1080 @ density 0.33 → K=1006.
        let k = photo_k_per_photo(1920, 1080, default_density());
        assert_eq!(k, 1006);
    }
    ...
    fn k_per_photo_4k() {
        ...
        let k = photo_k_per_photo(3840, 2160, default_density());
        assert_eq!(k, 4025);
    }
    ...
    fn k_per_photo_8k_natural() {
        ...
        let k = photo_k_per_photo(7680, 4320, default_density());
        assert_eq!(k, 16_100);
    }
    ...
    fn k_global_clamps_to_max_k() {
        ...
        let k = compute_k_global(&dims, default_density()).unwrap();
    ...
    fn k_global_empty_returns_none() {
        assert!(compute_k_global(&[], default_density()).is_none());
    }

    #[test]
    fn k_global_tiny_photo_returns_none() {
        // 1×1 pixel has only 3 channels; much less than plaintext header.
        assert!(compute_k_global(&[(1, 1)], default_density()).is_none());
    }
